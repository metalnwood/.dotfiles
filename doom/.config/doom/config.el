...
(setf maw/dash-case-hook
      (lambda () (cond ((and (eq (char-before) ?\s)
                             (eq (char-before (- (point) 1)) ?-))
                        (delete-backward-char 2)
                        (insert-char ?\s)
                        (remove-hook 'post-self-insert-hook  maw/dash-case-hook  t))
                       ((eq (char-before) ?\s)
                        (delete-backward-char 1)
                        (insert-char ?-)))))


(defun maw/set-dash-case ()
  (interactive)
  (add-hook 'post-self-insert-hook  maw/dash-case-hook nil t))

(map!  "C-;"   #'maw/set-dash-case)
(map! :leader
